    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className={`flex flex-col bg-white border-r border-gray-200 transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'}`}>
        ...
        <div className="flex items-center gap-3 px-4 py-4 border-b border-gray-200">
          ...
          {!collapsed && (
            <div>
              <h1 className="text-sm font-bold text-gray-900">WhatsApp CRM</h1>
              <p className="text-xs text-gray-500 truncate max-w-[130px]">{user?.organization?.name || 'Loading...'}</p>
            </div>
          )}
        </div>
          ...
          <div className="px-3 py-3 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="w-full pl-8 pr-3 py-1.5 text-xs bg-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-whatsapp-green/50"
              />
            </div>
          </div>
        )}
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 group
                  ${isActive ? 'bg-whatsapp-green/10 text-whatsapp-teal' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
                title={collapsed ? label : undefined}
              >
                <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-whatsapp-teal' : 'text-gray-500 group-hover:text-gray-700'}`} />
        ...
        <div className="border-t border-gray-200 p-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-2 py-1.5 text-xs text-red-600 hover:bg-red-50 rounded-lg transition-colors flex-1"
              title="Logout"
            >
